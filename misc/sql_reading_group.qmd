...
1. How are datetimes stored in MySQL?
1. What is the difference between `DATEDIFF` and `TIMESTAMPDIFF`?
1. What does `CURDATE` do?
1. Write a query that finds the average duration of a market.
1. Write a query that finds the average duration of a market for each market start time.
1. Write a query that finds the time between first and last daily purchase for customers with more than one purchase in a day.
